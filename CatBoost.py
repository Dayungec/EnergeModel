...
# 2. CatBoost模型类（保持不变）
# ----------------------------
class CatBoostSusceptibilityModel:
    ...
    def build_model(self, **catboost_params):
        """构建CatBoost模型"""
        default_params = {
            'iterations': 1000,
            'learning_rate': 0.05,
            ...
            'task_type': 'CPU',
            'train_dir': os.path.join(output_dir, 'catboost_info')  # 添加这一行
        }
    ...
    def predict_proba(self, X, calibrated=True):
        """预测概率（calibrated参数为了保持接口一致）"""
        if self.model is None:
            raise ValueError("模型尚未训练")

        # CatBoost本身已经经过良好校准，calibrated参数主要用于接口兼容
...
# ----------------------------
# 3. 基于CatBoost的易发性评价模型
# ----------------------------
class CatBoostModel:
    def __init__(self, input_dim: int, random_state: int = 42):
        self.input_dim = input_dim
        self.random_state = random_state
        self.catboost_model = CatBoostSusceptibilityModel(input_dim, random_state)
        self.feature_names = None
    ...
    def predict_proba(self, x, calibrated=True):
        ...
        if isinstance(x, torch.Tensor):
            x = x.numpy()
        return self.catboost_model.predict_proba(x, calibrated)
    ...
    def fit(self, X_pos, X_neg, feature_names):
        ...
        # 构建并训练模型
        self.catboost_model.build_model()
...
if __name__ == "__main__":
    ...
    output_dir = "result/catboost_regression"

    # 运行训练评估流程
    print("🚀 开始训练...")
    df_pos = DataReader.load_data(pos_path)

    model = CatBoostModel(input_dim=df_pos.shape[1])
    ...
    if Config.EXPORT_TIFF:
        ...
        risk_thresholds = processor.generate_susceptibility_zones(
