...
# 2. XGBoost模型类
# ----------------------------
class XGBoostSusceptibilityModel:
    ...
    def build_model(self, **xgb_params):
        """构建XGBoost模型"""
        default_params = {
            'n_estimators': 1000,
            'learning_rate': 0.05,
...
class XGBoostModel:
    def __init__(self, input_dim: int, random_state: int = 42):
        self.input_dim = input_dim
        self.random_state = random_state
        self.xgboost_model = XGBoostSusceptibilityModel(input_dim, random_state)
        self.feature_names = None
    ...
    def predict_proba(self, x, calibrated=True):
        ...
        if isinstance(x, torch.Tensor):
            x = x.numpy()
        return self.xgboost_model.predict_proba(x, calibrated)
    ...
    def fit(self, X_pos, X_neg, feature_names):
        ...
        self.xgboost_model.fit(X_train, y_train,X_train, y_train)
...
if __name__ == "__main__":
    ...
    output_dir = "result/results_xgboost"

    # 运行训练评估流程
    print("🚀 开始训练...")
    df_pos = DataReader.load_data(pos_path)

    model = XGBoostModel(input_dim=df_pos.shape[1])
