...
def load_data(pos_path: str, neg_path: str, bg_path: str, test_path: str = None) -> Tuple[
    ...
    # 加载负样本数据并按正样本列顺序对齐
    df_neg = DataReader.load_data(neg_path)

    # 验证负样本数据是否包含所有必需的列
    missing_in_neg = set(pos_columns_order) - set(df_neg.columns)
    if missing_in_neg:
        raise ValueError(f"负样本数据缺失以下列: {missing_in_neg}")

    df_neg = df_neg.reindex(columns=pos_columns_order)
    ...
    df_neg = df_neg.astype(np.float32)
    df_bg = df_bg.astype(np.float32)

    return df_pos, df_neg, df_bg, df_test, pos_columns_order
...
def calculate_risk_zones(model, df_bg, df_pos, feature_names,
    ...
    # 定义风险区域
    risk_names = ['高风险区', '中高风险区', '中风险区', '低风险区']
...
def evaluate_test_set(model, df_test, df_bg,df_neg,feature_names):
    if df_test is None:
        return None
    ...
    neg_probs = df_neg["原始概率"].values
    _, _, _, auc = calculate_pq_curve(
        positive_probs, neg_probs
    )
...
def train_and_evaluate(
    ...
    # 数据加载（包含测试数据）
    df_pos, df_neg, df_bg, df_test, feature_names = load_data(pos_path, neg_path, bg_path, test_path)

    # 转换为numpy数组用于训练
    X_pos = df_pos.values.astype(np.float32)
    X_neg = df_neg.values.astype(np.float32)
    X_bg = df_bg.values.astype(np.float32)
    X_test = df_test.values.astype(np.float32)

    print(f"正样本数量: {len(X_pos)}")
    print(f"负样本数量: {len(X_neg)}")
    ...
    # 创建并训练模型
    print("🚀 开始训练模型...")
    model.fit(X_pos, X_neg, feature_names)

    # 评估模型（增加测试集评估）
    results = evaluate_model(model, X_pos, X_neg,X_bg, X_test,df_pos, df_neg, df_bg, df_test, feature_names, output_dir)
...
def evaluate_model(
    ...
    df_neg["原始概率"] = model.predict_proba(X_neg)
    ...
    # 测试集评估（新增）
    test_metrics = evaluate_test_set(model, df_test, df_bg,df_neg,feature_names)

    # 保存结果
    df_pos_with_prob.to_excel(f"{output_dir}/positive_with_prob.xlsx", index=False)
    df_neg.to_excel(f"{output_dir}/negative_with_prob.xlsx", index=False)
    ...
    metrics = {
        ...
        "neg_prob_mean": df_neg["原始概率"].mean(),
        "pos_prob_std": df_pos["原始概率"].std(),
        "neg_prob_std": df_neg["原始概率"].std(),
        "pos_median": np.median(df_pos["原始概率"]),
        "neg_median": np.median(df_neg["原始概率"]),
    ...
    # 可视化（增加测试集结果展示）
    create_visualizations(model,df_pos, df_neg,df_bg, metrics, feature_importance_df, risk_results, output_dir, df_test)
...
# 7. 可视化函数（增加测试集展示）
# ----------------------------
def create_visualizations(model,df_pos, df_neg, df_bg,metrics, feature_importance_df, risk_results, output_dir,
                                  df_test=None):
    """创建可视化"""
    if df_test is not None:
    ...
    plt.hist(df_neg["原始概率"], bins=50, alpha=0.5, label=f"训练负样本 (n={len(df_neg)})", density=True)
    ...
    # 7. 概率箱线图
    plt.subplot(2, 4, 7)
    prob_data = [df_pos["原始概率"], df_neg["原始概率"]]
    if df_test is not None:
        # 添加测试集箱线图
        test_pos_probs = df_test[df_test.iloc[:, -1] == 1]["原始概率"] if len(df_test[df_test.iloc[:, -1] == 1]) > 0 else []
        test_neg_probs = df_test[df_test.iloc[:, -1] == 0]["原始概率"] if len(df_test[df_test.iloc[:, -1] == 0]) > 0 else []
        if len(test_pos_probs) > 0 and len(test_neg_probs) > 0:
            prob_data.extend([test_pos_probs, test_neg_probs])
            labels = ['训练正样本', '训练负样本', '测试正样本', '测试负样本']
        else:
            labels = ['训练正样本', '训练负样本']
    else:
        labels = ['训练正样本', '训练负样本']
